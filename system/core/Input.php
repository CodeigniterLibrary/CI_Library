...
abstract class CI_Input {
	 ...
	 * @var bool
	 */
	protected static $_standardize_newlines	= TRUE;
	/**
	 ...
	 * @var bool
	 */
	protected static $_enable_xss			= FALSE;
	/**
	 ...
	 * @var bool
	 */
	protected static $_enable_csrf			= FALSE;
	/**
	...
	public function __construct()
	{
		...
		self::$_allow_get_array	= (config_item('allow_get_array') === TRUE);
		self::$_enable_xss		= (config_item('global_xss_filtering') === TRUE);
		self::$_enable_csrf		= (config_item('csrf_protection') === TRUE);
	...
	protected static function _fetch_from_array(&$array, $index = '', $xss_clean = FALSE)
	{
		...
		if ($xss_clean === TRUE)
		{
			return self::$security->xss_clean($array[$index]);
		}
	...
	* @return	void
	*/
	public static function set_cookie($name = '', $value = '', $expire = '', $domain = '', $path = '/', $prefix = '', $secure = FALSE)
	{
		if (is_array($name))
		{
	...
	*/
	public function ip_address()
	{
		...
		if ( ! empty($proxy_ips))
		{
			...
			self::$ip_address = ($spoof !== FALSE && in_array($_SERVER['REMOTE_ADDR'], $proxy_ips, TRUE))
	...
	*/
	protected static function _sanitize_globals()
	{
		...
		if (self::$_enable_csrf == TRUE && ! self::is_cli_request())
		{
			self::$security->csrf_verify();
		}
	...
	*/
	protected static function _clean_input_data($str)
	{
		...
		*/
		...
		{
			$str = self::$security->xss_clean($str);
		}
	...
	* @return	string
	*/
	protected static function _clean_input_keys($str)
	{
	 ...
	 * @return array
	 */
	public static function request_headers($xss_clean = FALSE)
	{
		// Look at Apache go!
		...
		else
		{
			...
			foreach ($_SERVER as $key => $val)
			{
				...
				{
					$headers[substr($key, 5)] = self::_fetch_from_array($_SERVER, $key, $xss_clean);
				}
			}
		}
		...
		{
			$key = ucwords(
				str_replace(array('_', ' '), array(' ', '-'), strtolower($key))
			);
	 ...
	 *
	 * @param 	string		array key for self::$headers
	 * @param	boolean		XSS Clean or not
	 * @return 	mixed		FALSE on failure, string on success
	 */
	public static function get_request_header($index, $xss_clean = FALSE)
	{
		...
		if ($xss_clean === TRUE)
		{
			return self::$security->xss_clean(self::$headers[$index]);
		}
