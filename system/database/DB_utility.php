...
class CI_DB_utility extends CI_DB_forge {
	...
	function csv_from_result($query, $delim = ",", $newline = "\n", $enclosure = '"')
	{
		...
		// foreach ($query->result_array() as $row) if large
	...
	public function my_csv_from_result($query, $delim = ",", $newline = "\n", $enclosure = '"')
	{
		if ( ! is_object($query) OR ! method_exists($query, 'list_fields'))
		{
			show_error('You must submit a valid result object');
	...
	public function my_xml_from_result($query, $params = array())
	{
		if ( ! is_object($query) OR ! method_exists($query, 'list_fields'))
		{
			show_error('You must submit a valid result object');
		...
		while ($row = $query->row_arr())
		{
			echo $tab,"<{$element}>",$newline;

			foreach ($row as $key => $val)
			{
				echo $tab,$tab,"<{$key}>",xml_convert($val),"</{$key}>",$newline;
			}
			echo $tab,"</{$element}>",$newline;
		}
		echo "</$root>".$newline;
