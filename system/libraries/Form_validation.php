...
class CI_Form_validation {
	...
	public function set_rules($field, $label = '', $rules = '')
	{
		...
		if (is_array($field))
		{
			foreach ($field as $row)
			{
				...
				if (isset($row['field'])===FALSE OR isset($row['rules'])===FALSE)
				{
					continue;
				}
				else
		...
		if (is_string($field) === FALSE OR is_string($rules) === FALSE OR $field == '')
		{
			return $this;
		}
	...
	{
		if (isset($this->_field_data[$field]['error']) === FALSE OR $this->_field_data[$field]['error'] == '')
		{
			return '';
		}
		else
	...
	protected function _reduce_array($array, $keys, $i = 0)
	{
		if (is_array($array))
		{
			if (isset($keys[$i]))
			{
				...
				{
					return $this->_reduce_array($array[$keys[$i]], $keys, ($i+1));
				}
				else
				{
					return NULL;
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		$callback = FALSE;
		if (in_array('required', $rules) === FALSE AND is_null($postdata))
		{
		...
		if (is_null($postdata) AND $callback == FALSE)
		{
			if (in_array('isset', $rules, TRUE) OR in_array('required', $rules))
			{
				...
				if (isset($this->_error_messages[$type]) === FALSE)
				{
				...
				if (isset($this->_error_array[$row['field']]) === FALSE)
				{
		...
		foreach ($rules As $rule)
		{
			...
			if ($row['is_array'] == TRUE AND is_array($this->_field_data[$row['field']]['postdata']))
			{
				...
				if (isset($this->_field_data[$row['field']]['postdata'][$cycles]) === FALSE)
				{
					continue;
				}
				else
			...
			if ($callback === TRUE)
			{
				if (method_exists($this->CI, $rule) === FALSE)
				{
					continue;
				}
				...
				if (in_array('required', $rules, TRUE) === FALSE AND $result !== FALSE)
				{
					continue;
				}
			}
			...
			{
				if (method_exists($this, $rule) === FALSE)
				{
			...
			if ($result === FALSE)
			{
				if (isset($this->_error_messages[$rule]) === FALSE)
				{
	...
	{
		if (isset($this->_field_data[$field]) === FALSE)
		{
			return $default;
		}
		// If the data is an array output them one at a time.
		//     E.g: form_input('name[]', set_value('name[]');
		elseif (is_array($this->_field_data[$field]['postdata']))
		{
	...
	{
		if (isset($this->_field_data[$field]) === FALSE OR isset($this->_field_data[$field]['postdata']) === FALSE)
		{
	...
	{
		if (isset($this->_field_data[$field]) === FALSE OR
			isset($this->_field_data[$field]['postdata']) === FALSE)
		{
			if ($default === TRUE AND count($this->_field_data) === 0)
			{
	 ...
	 * @return	bool
	 */
	protected function is_unique($str, $field)
	{
	 ...
	 * @return	bool
	 */
	protected function valid_emails($str)
	{
		if (strpos($str, ','))
		{
	 ...
	 * @return	string
	 */
	protected function prep_for_form($data = '')
	{
		if (is_array($data))
		{
		...
		elseif ($this->_safe_form_data == FALSE OR $data === '')
		{
			return $data;
		}
		else
	 ...
	 * @return	string
	 */
	protected function strip_image_tags($str)
	{
	 ...
	 * @return	string
	 */
	protected function xss_clean($str)
	{
