...
class MY_Router extends CI_Router {
	...
	public function _validate_request($segments)
	{
		...
		elseif ($segments[0] === 'module')
		{
			return $this->_validate_module_request($segments);
		}
		elseif (file_exists(APPPATH.'controllers/'.$segments[0].'.php'))
		...
		{// Is the controller in a sub-folder?
			...
			// $this->_set_directory($segments[0]);
			// $segments = array_slice($segments, 1);
            $tempDir = array();
            $i = 0;

            for($i_ = count($segments); $i < $i_; ++$i)
            {
            ...
            $this->_set_directory(implode('/', $tempDir));
	 ...
	 * @return	array
	 */
	private function _validate_module_request($segments)
	{
		$segments = array_slice($segments, 1);

		$this->set_directory(APPPATH.'third_party');
	...
	protected function _set_request($segments = array())
	{
		...
		if (isset($segments[0]) && $segments[0] === 'module')
		{
	 ...
	 * @return	void
	 */
	private function _set_directory($dir)
	{
		$this->directory = $dir.'/';
	}
