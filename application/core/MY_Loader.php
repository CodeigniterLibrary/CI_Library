...
class MY_Loader extends CI_Loader {
	...
	private $_ci_is_inside_module = false;  // 当前是否是 Module 里的 Loader
	private $_ci_module_path = '';  // 当前 Module 所在路径
	private $_ci_module_class = '';  // 当前 Module 的控制器名
	...
	private $_ci_module_method = '';	// 当前 Module 执行的方法
	public $_ci_module_models = array();	// 用于通过模型类名反查属于哪个模块
	 ...
	 * @return  void
	 */
	protected function module($module_uri, $vars = array(), $return = FALSE)
	{
		if ($module_uri == '')
		{
			return;
		...
		$class_name = str_replace(
						' ', '_', ucwords(
								str_replace('/', ' ', $path . $module . ' ' . str_replace(' ', '_', ucwords(str_replace('_', ' ', $controller))))
						)
				) . '_module';
	 ...
	 * @return  string
	 */
	protected function module_url($uri, $controller_name = '')
	{
		$CI = & get_instance();
	...
	public function library($library = '', $params = NULL, $object_name = NULL)
	{
		...
		elseif ($library == '' OR isset($this->_base_classes[$library]))
		{
			return FALSE;
		}
		elseif (!(is_null($params) OR is_array($params)))
		{
			$params = NULL;
		};
